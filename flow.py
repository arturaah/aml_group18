...
class MaskedCouplingLayer(nn.Module):
    ...
    def forward(self, z):
        ...
        # Pass the masked part through the scaling and translation networks
        scale = self.scale_net(masked_z) 
        translation = self.translation_net(masked_z) 
        
        z_exp = z * torch.exp(scale) + translation
        
        x = self.mask*z+(1-self.mask)*z_exp
    ...
    def inverse(self, x):
        ...
        z = self.mask*x+(1-self.mask)*((x - translation)*torch.exp(-scale))
...
def masking_strategy(mask_strat,D):
    if mask_strat == 'vanilla':
        # Do the old stuff
        # Make a mask that is 1 for the first half of the features and 0 for the second half
        mask = torch.zeros((D,))
        mask[D//2:] = 1
    elif mask_strat == 'random':
        mask = torch.randint(0, 2, (D,)).float() 
    else: # mask_strat == 'chequerboard'
        n = int(np.sqrt(D))
        mask = torch.tensor([(i + j) % 2 for i in range(n) for j in range(n)], dtype=torch.float32)
    return mask
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', type=str, default='model_new.pt', help='file to save model to or load model from (default: %(default)s)')
    ...
    # D = next(iter(train_loader)).shape[1] ## THIS CORROSPONDS TO FEATURE_DIM!! 2 for 2D data, 784 for flatten MNIST
    ...
    if args.mode == 'train':
        ...
        samples = samples.view(-1, 1, 28, 28)  # Add channel dimension for save_image
